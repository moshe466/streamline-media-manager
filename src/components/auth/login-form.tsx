...
export function LoginForm({ showSignUp = true }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success && result.role && result.activeSessionId) {
        ...
        // Persistent memory of which login page was used
        if (typeof window !== 'undefined') {
            localStorage.setItem('loginEntryPoint', window.location.pathname.includes('/uh') ? 'uh' : 'standard');
        }
