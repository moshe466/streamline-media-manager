...
import { AdminStreamsHeroV2 } from '@/components/dashboard-v2/admin-streams-hero-v2';
import { StreamCardV2 } from '@/components/dashboard-v2/stream-card-v2';
import { AdminBottomPanelsV3 } from '@/components/dashboard-v2/admin-bottom-panels-v3';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function StreamActions({ streams: initialStreams, error = null, userType = 'admin', clientId }: StreamActionsProps) {
  ...
  const totalViewers = useMemo(() => streams.reduce((sum, stream) => sum + Number((stream as any)?.stats?.client_count ?? (stream as any)?.stats?.clients ?? 0), 0), [streams]);
  ...
  return (
    <div className={cn("space-y-8 text-right", userType === "admin" && "admin-control-bg -m-4 sm:-m-6 lg:-m-8 p-4 sm:p-6 lg:p-8 min-h-[calc(100vh-4rem)]")}>
        ...
        onCreateClick={
          <Dialog open={dialogOpen} onOpenChange={(isOpen) => { if (!isOpen) resetForm(); setDialogOpen(isOpen); }}>
            <DialogTrigger asChild>
              <Button data-tour="streams-create-button" className="w-full xl:w-auto rounded-full bg-cyan-500 hover:bg-cyan-400 shadow-lg shadow-violet-900/30">
      ...
      />
       ...
       <Card className={cn(userType === "admin" && "admin-glass-card border-white/10")}>
        <CardHeader>
                    ...
                    {filteredStreams.map(stream => {
                        ...
                        return (
                            <StreamCardV2
                                ...
                                userType={userType}
                                canDeleteStreams={canDeleteStreams}
                                actioningStream={actioningStream}
                                onOpenPreview={handleOpenPreview}
                                onDeleteStream={handleDeleteStream}
                                managementLink={getStreamManagementLink(stream.name)}
                            />
                        )
                    })}
                </div>
