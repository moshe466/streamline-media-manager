...
export default function LiveBroadcastPage() {
        ...
        const fetchAndFilterStreams = async (isInitialLoad = false) => {
            ...
            try {
                if (role === 'admin' || role === 'super-admin') {
                    ...
                    // Broadcasters see only their permitted streams
                    const broadcasterId = sessionStorage.getItem('userId');
                    if (!broadcasterId) {
                        throw new Error("Broadcaster not identified.");
                    }

                    const client = await getClientById(broadcasterId);
                    if (!client) {
                        throw new Error("Associated client not found for broadcaster.");
                    }
    ...
    const handleStartBroadcast = (streamName: string) => {
        router.push(`/admin/live-broadcast/${streamName}`);
    };

    const pageTitle = userRole === 'admin' || userRole === 'super-admin'
        ? "כלל השידורים במערכת"
        : "שידור חי מהמכשיר";

    const pageDescription = userRole === 'admin' || userRole === 'super-admin'
