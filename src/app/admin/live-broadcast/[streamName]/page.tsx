...
export default function BroadcastCapturePage() {
    ...
    const handleToggleScreenShare = async () => {
        ...
        try {
            ...
            screenVideoTrack.onended = () => {
             ...
             const microphoneAudioTrack = mediaStreamRef.current?.getAudioTracks()[0];
             const combinedAudioTracks = [microphoneAudioTrack, screenAudioTrack].filter(Boolean) as MediaStreamTrack[];

            mediaStreamRef.current = new MediaStream([screenVideoTrack, ...combinedAudioTracks]);
    ...
    const handleToggleStream = async () => {
        ...
        try {
            ...
            const videoTrack = mediaStreamRef.current.getVideoTracks()[0];
            const audioTrack = audioProcessingRef.current?.destinationNode.stream.getAudioTracks()[0];
            
            if (videoTrack) pc.addTrack(videoTrack, mediaStreamRef.current);
            if (audioTrack) pc.addTrack(audioTrack, audioProcessingRef.current!.destinationNode.stream);
            ...
            <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-start z-10 bg-gradient-to-b from-black/50 to-transparent">
                 <div className="flex-1 flex justify-start">
                    {isStreaming ? (
                         ...
                         <Button asChild variant="outline" className="bg-black/30 border-white/20"><Link href="/admin/live-broadcast"><ArrowRight className="ml-2 h-4 w-4" />חזור</Link></Button>
                    )}
                </div>
