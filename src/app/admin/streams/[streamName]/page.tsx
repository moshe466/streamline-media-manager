...
import { ArrowRight, Power, PowerOff, Save, Loader2, PlusCircle, Trash2, Video, Clapperboard, Play, StopCircle, Edit, CheckSquare, FileSignature, Copy, ClipboardCheck, Users, HardDrive, History, ZapOff, Settings2, AlertTriangle, ArrowRightLeft, RadioTower, Image as ImageIcon, Eye, EyeOff, Link2, Activity, Globe, Zap, Youtube, Facebook } from "lucide-react";
...
export default function StreamDetailsPage() {
    ...
    const [newStreamName, setNewStreamName] = useState(currentStreamName);
    ...
    const handleRefetchDetails = useCallback(async () => {
        try {
            ...
            // Sync editable pushes from server if not currently editing (not perfectly ideal but works for basic sync)
            if (!isSaving && details.pushes) {
                setEditablePushes(details.pushes.map((p, i) => ({ ...p, id: p.url || `push_${i}`, ...parsePushUrl(p.url), name: p.comment || 'יעד קיים' })));
            }
        } catch (error) {
            console.error("Error refetching stream details:", error);
        }
    }, [currentStreamName, isSaving]);
    ...
    const loadInitialData = useCallback(async () => {
        ...
        try {
            ...
            if (!details) {
                toast({ variant: 'destructive', title: 'שגיאה', description: 'לא ניתן למצוא את השידור.' });
                setIsLoading(false);
                return;
            }
    ...
    const handleEditMetadata = async () => {
        ...
        setIsSaving(true);
        const instanceId = sessionStorage.getItem('instanceId') || 'default';
        const result = await updateStreamMetadata(auth, currentStreamName, currentStreamName, currentSourceUrl, undefined, instanceId);
        setIsSaving(false);
    ...
    const updateLiveConfig = async (config: Partial<StreamDetails>, successMessage: string) => {
        ...
        setIsSaving(true);
        const instanceId = sessionStorage.getItem('instanceId') || 'default';
        const result = await updateStream(auth, currentStreamName, config, undefined, instanceId);
        setIsSaving(false);
    ...
    const handleDeleteSecureLink = async (id: string) => {
        ...
        const actorName = sessionStorage.getItem('userNickname') || 'מנהל';
    ...
    if (!liveData.details) return <div className="text-center p-10">שידור לא נמצא.</div>;
    ...
    // Live Push Status processing
    const activePushes: any[] = Array.isArray(liveData.details.stats?.push) ? liveData.details.stats?.push : [];

    return (
        <div className="space-y-6 text-right">
            <div className="flex flex-col-reverse sm:flex-row-reverse items-center justify-between gap-4">
                <Button asChild variant="outline">
                    <Link href="/admin/streams"><ArrowRight className="ml-2 h-4 w-4" />חזרה</Link>
                </Button>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight flex items-center gap-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                <Card data-tour="stream-preview-card" className="flex flex-col h-full lg:col-span-1">
                    <CardHeader>
                        <div className="flex justify-between items-center">
                            ...
                            <Badge variant={isStreamOnline ? 'default' : 'destructive'} className={cn("text-lg py-1 px-3", isStreamOnline ? 'bg-green-600' : 'bg-red-600')}>
                                {isStreamOnline ? 'אונליין' : 'אופליין'}
                            </Badge>
                        </div>
                    </CardHeader>
                ...
                <Card data-tour="stream-stats-card" className="flex flex-col h-full lg:col-span-2">
                    ...
                    <CardContent className="p-0 flex-grow">
                        {isStreamOnline ? <StreamStatsChart streamName={currentStreamName} initialData={liveData.details} /> : <div className="flex items-center justify-center h-full text-muted-foreground">שידור לא פעיל</div>}
                    </CardContent>
                </Card>
            </div>
                ...
                <CardContent>
                    {activePushes.length > 0 ? (
                        <div className="grid gap-4 sm:grid-cols-2">
                            {activePushes.map((p: any, i: number) => (
                                <div key={i} className="p-4 border rounded-lg bg-card space-y-2 text-right">
                                    <div className="flex items-center justify-between">
                                        <Badge variant={p.status === 'running' ? 'default' : 'secondary'} className={p.status === 'running' ? 'bg-green-600' : ''}>
                                            {p.status === 'running' ? 'מזרים' : p.status}
                                        </Badge>
                                        <span className="font-bold flex items-center gap-2">
                                            {p.url.includes('youtube') ? <Youtube className="h-4 w-4 text-red-500" /> : p.url.includes('facebook') ? <Facebook className="h-4 w-4 text-blue-500" /> : <Globe className="h-4 w-4" />}
                                            יעד #{i+1}
                                        </span>
                                    </div>
                                    <p className="text-xs font-mono truncate dir-ltr text-left opacity-70" title={p.url}>{p.url}</p>
                                    <Separator />
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                        <div className="flex justify-between"><span>{p.bitrate || 0} kbps</span><span className="text-muted-foreground">Bitrate:</span></div>
                                        <div className="flex justify-between"><span>{(p.bytes / 1e6).toFixed(1)} MB</span><span className="text-muted-foreground">Sent:</span></div>
                                        <div className="flex justify-between text-red-400"><span>{p.errors_dropped_frames || 0}</span><span className="text-muted-foreground">Dropped:</span></div>
                                        <div className="flex justify-between"><span>{p.uptime ? `${Math.floor(p.uptime / 60)}m` : 'N/A'}</span><span className="text-muted-foreground">Uptime:</span></div>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="text-center py-8 text-muted-foreground border border-dashed rounded-lg">
                            <ZapOff className="mx-auto h-8 w-8 opacity-50 mb-2" />
                            <p>אין הזרמות Push פעילות כרגע.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-center">
                        <Button variant="outline" onClick={() => setEditablePushes(p => p.length < 10 ? [...p, {id: `new_${Date.now()}`, url: '', name: 'יעד חדש', rtmp_url: '', stream_key: ''}] : p)}>
                            <PlusCircle className="ml-2 h-4 w-4" />
                            הוסף יעד הזרמה
                        </Button>
                        <div className="space-y-1.5 text-right w-full">
                            <h2 className="text-2xl font-bold tracking-tight">ניהול יעדי הזרמה (Push)</h2>
                            <p className="text-muted-foreground text-sm">הוסף כתובות RTMP חיצוניות. המידע יסונכרן ישירות עם השרת.</p>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4">
                    {editablePushes.length > 0 ? (
                        <div className="grid gap-6">
                            {editablePushes.map((push, index) => (
                                <Card key={push.id} className="bg-muted/30 p-4 border-dashed">
                                    <div className="flex items-center justify-between mb-4">
                                        <Button variant="destructive" size="icon" onClick={() => setEditablePushes(p => p.filter(item => item.id !== push.id))}>
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                        <div className="space-y-2">
                                            <Label htmlFor={`push-name-${index}`}>שם/תיאור היעד</Label>
                                            <Input id={`push-name-${index}`} value={push.name || ''} onChange={(e) => setEditablePushes(p => p.map(item => item.id === push.id ? {...item, name: e.target.value} : item))} dir="rtl" placeholder="למשל: YouTube Live" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label htmlFor={`push-url-${index}`}>כתובת שרת (RTMP URL)</Label>
                                            <Input id={`push-url-${index}`} placeholder="rtmp://a.rtmp.youtube.com/live2" value={push.rtmp_url || ''} onChange={(e) => setEditablePushes(p => p.map(item => item.id === push.id ? {...item, rtmp_url: e.target.value} : item))} className="font-mono text-xs" dir="ltr"/>
                                        </div>
                                        <div className="space-y-2 md:col-span-2">
                                            <Label htmlFor={`push-key-${index}`}>מפתח הזרמה (Stream Key)</Label>
                                            <Input id={`push-key-${index}`} placeholder="xxxx-xxxx-xxxx-xxxx" value={push.stream_key || ''} onChange={(e) => setEditablePushes(p => p.map(item => item.id === push.id ? {...item, stream_key: e.target.value} : item))} className="font-mono text-xs" dir="ltr"/>
                                        </div>
                                    </div>
                                </Card>
                            ))}
                        ...
                        <div className="text-center py-12 text-muted-foreground bg-muted/20 rounded-lg border border-dashed">
                            <RadioTower className="mx-auto h-12 w-12 opacity-20 mb-4" />
                            <p>לא הוגדרו יעדי הזרמה לערוץ זה.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
            ...
            <div className="flex justify-start pt-6 mt-6 border-t sticky bottom-0 bg-background/90 backdrop-blur-md p-4 z-20 rounded-t-xl border-x border-t">
                <Button 
                    onClick={() => updateLiveConfig({ 
                        pushes: editablePushes.map(p => ({ 
                            url: combinePushUrl(p.rtmp_url, p.stream_key), 
                            comment: p.name || '' 
                        })) 
                    }, "הגדרות השידור והזרמת ה-Push עודכנו בהצלחה.")} 
                    disabled={isSaving} 
                    className="w-full md:w-auto min-w-[200px] h-12 shadow-lg"
                >
                    {isSaving ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Save className="mr-2 h-5 w-5" />}
                    שמור את כל השינויים בערוץ
                </Button>
            </div>
        </div>
    );
