        ...
        <div className="h-full flex flex-col p-4 sm:p-6 lg:p-8 space-y-4 text-right">
                ...
                <DialogContent className="sm:max-w-2xl text-right"><DialogHeader><DialogTitle>הגדרות תצוגה מרובה</DialogTitle><DialogDescription>בחר אילו שידורים להציג וקבע את פריסת הרשת.</DialogDescription></DialogHeader>
                    <div className="py-4 space-y-6">
                        ...
                        <div>
                            ...
                            <ToggleGroup type="single" value={String(tempGridColumns)} onValueChange={(val) => val && setTempGridColumns(Number(val))} className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="secondary" onClick={handleGenerateAndOpen} disabled={isPreparingWindow}>
                        {isPreparingWindow ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <ExternalLink className="ml-2 h-4 w-4" />}
                        לצפיה בחלון נפרד
                    </Button>
                    <Button onClick={handleOpenSettings} variant="outline"><Settings className="ml-2 h-4 w-4" />הגדרות</Button>
                </div>
            </div>
            
            {selectedStreamNames.length > 0 ? (
                <div className="grid gap-2 flex-1" style={{ gridTemplateColumns }}>
                    {Array.from({ length: selectedStreamNames.length }).map((_, index) => (
                        <div key={selectedStreamNames[index]} className="aspect-video relative group bg-muted/50 rounded-md flex flex-col items-center justify-center border-2 border-dashed">
                           <Tv className="h-8 w-8 text-muted-foreground"/>
                           <p className="text-xs text-muted-foreground mt-2">{selectedStreamNames[index]}</p>
                        </div>
                    ))}
                </div>
            ) : (
